...
function start(){scan();document.documentElement.setAttribute('data-ds-related-status',document.querySelector('.ds-related')?'mounted':'watching');new MutationObserver(()=>queue([document])).observe(document.body,{childList:true,subtree:true,attributes:true,attributeFilter:['data-product-uid','data-product-lid','data-product-url','href']});window.addEventListener('popstate',()=>setTimeout(scan,100));window.addEventListener('hashchange',()=>{setTimeout(scan,50);setTimeout(scan,300)});document.addEventListener('click',e=>{if(e.target.closest?.('a[href*="/tproduct/"]')){setTimeout(scan,100);setTimeout(scan,400);setTimeout(scan,800)}})}
