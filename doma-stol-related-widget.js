(function(){
const DB=window.DS_RELATED_DB;if(!DB)return;
...
function start(){scan();new MutationObserver(ms=>queue(ms.flatMap(m=>[...m.addedNodes]).filter(n=>n.nodeType===1))).observe(document.body,{childList:true,subtree:true});window.addEventListener('popstate',()=>setTimeout(scan,100));window.addEventListener('hashchange',()=>setTimeout(scan,100));document.addEventListener('click',e=>{if(e.target.closest?.('a[href*="/tproduct/"]'))setTimeout(scan,450)})}
